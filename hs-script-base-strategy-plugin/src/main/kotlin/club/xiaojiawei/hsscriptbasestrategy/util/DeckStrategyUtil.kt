...
//        清理嘲讽
        ...
        if (findTauntCardCount > 0) {
            ...
            if (execAction) {
                Thread.sleep(RandomUtil.getActionInterval(3500).toLong())
...
//                敌方随从能被攻击，突袭无法攻击英雄
    ...
    fun calcPowerOrder(
        ...
        // Power.log can briefly expose a negative usable-resource value while a
        // turn is being reconstructed.  Never use that transient value as an
        // array size; treating it as zero lets the phase settle and prevents a
        // strategy-thread crash.
        val safeTarget = target.coerceAtLeast(0)
        if (target < 0) {
            log.warn { "计算出牌顺序时收到负资源值 $target，按 0 处理" }
        }
        // dp[j] 表示总 cost 为 j 时的最高 (cost + weight) 值
        val dp = DoubleArray(safeTarget + 1)
        val chosenCards = Array(safeTarget + 1) { mutableListOf<SimulateWeightCard>() }

        for (card in cards) {
            for (j in safeTarget downTo card.card.cost) {
        ...
        return Pair(dp[safeTarget], chosenCards[safeTarget].toSet().toList())
    }
    ...
    fun powerCard(
        ...
        if (coinCard != null) {
            ...
            if (coinScore > score) {
                ...
                outCard(coinResultCards, me)
                return
            }
        }
        outCard(resultCards, me)
    }

    /**
     * 使用手牌
     */
    private fun outCard(cards: List<SimulateWeightCard>, player: Player) {
        if (cards.isNotEmpty()) {
            val sortCard = sortCardByPowerWeight(cards)
            updateTextForCard(sortCard)
            log.info { "待出牌：$sortCard" }
            // Use the player snapshot supplied by this turn instead of the
            // mutable object-level cache populated by cleanPlay(). During an
            // E2E log replay another listener can briefly clear WAR.me, which
            // previously made this line throw NPE and silently skipped the
            // rest of the turn.
            val me = player
            for (simulateWeightCard in sortCard) {
                ...
                if (me.usableResource >= card.cost) {
                    ...
                    log.info {
                        "CARD_PLAY_SKIPPED reason=INSUFFICIENT_MANA card=${card.entityName.ifBlank { card.cardId }} " +
                            "cardId=${card.cardId} cost=${card.cost} currentMana=${me.usableResource}"
                    }
                }
            }
        }
