...
Dedicated replay/evaluator checkpoint：`PirateWarriorOfflineReplayTest` 3/3 passed；与上述两组相关测试合计 28/28 passed。7 个 synthetic fixtures、两局 LOST/一局 WON online-like scenarios，以及 12 个 A/B calibration rows 均打印策略、候选或 action、理由和预期/实际选择；模型 evaluator/verdict 均为 `pass`，runtime review 均为 `needs-review`，没有真实 E2E 结论。
