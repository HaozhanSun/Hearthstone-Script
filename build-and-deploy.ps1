...
if (-not $SkipTests) {
    $testArgs = $mavenBaseArgs + @('-DforkCount=0', '-Dtest=CardTimingPolicyTest,SurrenderPolicyTest,GameUtilSurrenderGuardTest,ScreenStateRecoveryTest,TurnEndActionGuardTest,PirateDemonHunterMctsExperimentModelTest,HsPirateDemonHunterDeckStrategyTest', '-Dsurefire.failIfNoSpecifiedTests=false', 'test')
