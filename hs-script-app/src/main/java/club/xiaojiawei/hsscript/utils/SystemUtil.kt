...
object SystemUtil {
    ...
    fun deleteAllContent() {
        ...
        delay(RandomUtil.getInteractionDelay(200))
     ...
     */
    fun shutdownSoft() {
        LifecycleTrace.mark("shutdown-request thread=${Thread.currentThread().name}")
        // The automated E2E runner must not be terminated before it has
        // recorded a script-controlled game result.  This guard also makes
        // an unexpected shutdown request observable instead of turning into
        // a silent watchdog restart.
        if (System.getProperty("hs.script.e2e") == "true" && !E2ETrace.resultRecorded) {
            val message = "E2E_SHUTDOWN_BLOCKED pid=${ProcessHandle.current().pid()} thread=${Thread.currentThread().name}"
        ...
        log.info { "准备关闭软件..." }
        LifecycleTrace.stop("shutdownSoft")
        runCatching {
