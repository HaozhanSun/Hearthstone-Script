...
object PowerLogUtil {
    ...
    private fun delayAfterChoice() {
        // The E2E harness already serializes real Robot input.  Holding the
        // global mouse lock while sleeping here can make the next click wait
        // at the last CLICK_ATTEMPT marker and look like a process exit.
        if (System.getProperty("hs.script.e2e") == "true") {
            log.info { "E2E_INPUT_LOCK_DELAY_SKIPPED" }
            return
        }
        DRIVER_LOCK.lock()
        try {
            SystemUtil.delay(RandomUtil.getActionInterval(2000))
        } finally {
            DRIVER_LOCK.unlock()
        }
    }
    ...
    /**
     * Power.log may publish a TAG_CHANGE before the matching FULL_ENTITY/SHOW_ENTITY.
     * Keep those changes briefly so parser ordering does not lose card state.
     */
    private val pendingTagChanges: MutableMap<String, ArrayDeque<TagChangeEntity>> = ConcurrentHashMap()

    private const val MAX_PENDING_TAG_CHANGES = 512
...
//        处理复杂，例：TAG_CHANGE Entity=[entityName=UNKNOWN ENTITY [cardType=INVALID] id=89 zone=HAND zonePos=4 cardId= player=2] tag=ZONE_POSITION value=0
            if (tagChangeEntity.entity.isBlank()) {
                val card = war.cardMap[tagChangeEntity.entityId] ?: let {
                    deferTagChange(tagChangeEntity)
                    return tagChangeEntity
                }

                applyTagChange(card, tagChangeEntity)
            } else {
//            处理简单，例：TAG_CHANGE Entity=BouncyBear tag=NUM_TURNS_LEFT value=1
    ...
    /** Called when a new Power.log stream is attached. */
    fun resetPendingTagChanges() {
        pendingTagChanges.clear()
    }

    private fun deferTagChange(tagChangeEntity: TagChangeEntity) {
        val queue = pendingTagChanges.computeIfAbsent(tagChangeEntity.entityId) { ArrayDeque() }
        synchronized(queue) {
            if (queue.size >= MAX_PENDING_TAG_CHANGES) {
                queue.removeFirst()
            }
            queue.addLast(tagChangeEntity)
        }
        log.debug { "TAG_CHANGE先于card实体到达，暂存【entityId:${tagChangeEntity.entityId}，tag:${tagChangeEntity.tag}】" }
    }

    private fun applyPendingTagChanges(card: Card) {
        val queue = pendingTagChanges.remove(card.entityId) ?: return
        synchronized(queue) {
            while (queue.isNotEmpty()) {
                applyTagChange(card, queue.removeFirst())
            }
        }
        log.debug { "已补应用暂存TAG_CHANGE【entityId:${card.entityId}】" }
    }

    private fun applyTagChange(card: Card, tagChangeEntity: TagChangeEntity) {
        // 只列出可能被修改的属性
...
//            [entityName=UNKNOWN ENTITY [cardType=INVALID] id=4 zone=DECK zonePos=0 cardId= player=1]
    ...
    fun isRelevance(l: String): Boolean {
        // Preserve action-result evidence even when the normal phase parser
        // filters this raw line out of the in-memory model update path.
        PowerActionEvidence.observeLine(l)
        var flag = false
        if (l.contains("Truncating log")) {
            ...
            // Recent Hearthstone clients emit the top-level turn STEP changes
            // from GameState.DebugPrintPower instead of PowerTaskList. Keep
            // consuming PowerTaskList for entity/block parsing, but also pass
            // through GameState's GameEntity STEP lines so the phase machine
            // can leave mulligan and observe MAIN_* / MAIN_END transitions.
            flag = l.contains("PowerTaskList") ||
                (l.contains("GameState.DebugPrintPower") &&
                    l.contains("TAG_CHANGE") &&
                    (l.contains("Entity=GameEntity tag=STEP") ||
                        l.contains("tag=MULLIGAN_STATE")))
        }
