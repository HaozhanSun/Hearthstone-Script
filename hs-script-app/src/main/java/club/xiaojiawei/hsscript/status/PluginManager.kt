        ...
        for (deckClassLoader in deckClassLoaders) {
            try {
                ...
                if (plugins.isNotEmpty()) {
                    ...
                    val pluginSource = plugin::class.java.protectionDomain?.codeSource?.location?.toExternalForm()
                    log.info {
                        "外部插件代码来源：id=${plugin.id()} version=${plugin.version()} " +
                            "source=${pluginSource ?: "<unknown>"} " +
                            "classLoader=${deckClassLoader?.javaClass?.name ?: "<null>"}"
                    }
