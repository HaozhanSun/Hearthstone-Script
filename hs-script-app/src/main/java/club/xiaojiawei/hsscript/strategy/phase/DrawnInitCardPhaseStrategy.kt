...
import club.xiaojiawei.hsscript.strategy.phase.ReplaceCardPhaseStrategy.handleMulliganInput
...
object DrawnInitCardPhaseStrategy : AbstractPhaseStrategy() {
    ...
    override fun dealTagChangeThenIsOver(line: String, tagChangeEntity: TagChangeEntity): Boolean {
        ...
        } else if (tagChangeEntity.tag == TagEnum.MULLIGAN_STATE
            && tagChangeEntity.value == MulliganStateEnum.INPUT.name
        ) {
            // Some current clients emit INPUT before BEGIN_MULLIGAN. Do not
            // lose that event while still in the initial-draw phase.
