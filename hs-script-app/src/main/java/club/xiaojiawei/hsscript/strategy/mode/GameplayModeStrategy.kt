    ...
    override fun afterEnter(t: Any?) {
        if (prevMode == ModeEnum.LOGIN || prevMode == null) {
            if (!WarEx.inWar) {
                ...
                // GAMEPLAY but has no active War is to reset and wait for a
                // fresh scene/log transition.  This is a guarded no-op, not
                // a reason to pause the whole script.
                log.error {
                    "GAMEPLAY状态无活动对局，拒绝盲目点击并等待新鲜日志 " +
