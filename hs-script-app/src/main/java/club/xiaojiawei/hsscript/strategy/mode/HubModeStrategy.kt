...
object HubModeStrategy : AbstractModeStrategy<Any?>() {
     ...
     * 新版主界面推荐/促销页的返回按钮。
     *
     * 这类页面仍然会被日志识别为主界面，但旧的三个弹窗关闭坐标
     * 都落在内容区，无法让页面回到模式入口，导致一直重复点击。
     */
    val HOME_PROMO_BACK_RECT by lazy {
    ...
    private fun ocrTask(frameReader: FrameReader): GameTaskRes? {
        ...
        for (i in 0 until 10) {
            ...
            if (frame != null) break
            SystemUtil.delay(randomizedTaskDelay(100).toInt())
        }
    ...
    private fun handleTask() {
        ...
        SystemUtil.delay(randomizedTaskDelay(2000).toInt())
        ...
        try {
            ...
            SystemUtil.delay(randomizedTaskDelay(4000).toInt())
            ...
            SystemUtil.delay(randomizedTaskDelay(1000).toInt())
    ...
    override fun afterEnter(t: Any?) {
        ...
        addEnteredTask(EXTRA_THREAD_POOL.scheduleWithFixedDelay({
            ...
            SystemUtil.delayShortMedium()
        }, randomizedTaskDelay(5000), randomizedTaskInterval(2000), TimeUnit.MILLISECONDS))
