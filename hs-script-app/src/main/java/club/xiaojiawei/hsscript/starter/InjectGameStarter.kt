...
class InjectGameStarter(
    ...
    override fun execStart() {
        ...
        val skipNativeInjection = RuntimeSafety.safeNative ||
            System.getProperty("hs.script.e2e.skip-inject") == "true"
//        val reductionGameWindowSize = ConfigEnum.GAME_WINDOW_REDUCTION_FACTOR.service?.getStatus(null) == true
        ...
        if (skipNativeInjection) {
            log.warn { "安全运行：跳过游戏 DLL 注入，使用 Java 输入与日志路径" }
        } else if (force ||
            mouseHook ||
            acHook ||
            limitMouseRange ||
