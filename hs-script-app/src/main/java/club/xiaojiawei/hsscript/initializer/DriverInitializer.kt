...
class DriverInitializer : AbstractInitializer() {
    ...
    override fun exec() {
        if (RuntimeSafety.safeNative) {
            // The E2E harness uses java.awt.Robot and must not load the
            // interception/csystem native driver. A driver load can terminate
            // the JVM from native code without an exception or shutdown hook.
            log.info { "安全运行：跳过鼠标驱动初始化" }
            return
        }
