...
abstract class AbstractLogListener(
    ...
    fun listen() {
        synchronized(this) {
            ...
            closeLogFile()
            // A clean E2E run may have to wait for Hearthstone's launcher,
            // login screen, and first game-log directory.  The normal 15s
            // timeout is useful for ordinary operation, but it is too short
            // for a cold start and turns a healthy launch into an automatic
            // pause before the game has had a chance to create Decks.log.
            val waitMillis = if (System.getProperty("hs.script.e2e") == "true") {
                60_000L
            } else {
                15_000L
            }
            val waitLogFile = waitLogCreated(waitMillis)
            if (waitLogFile == null){
