        ...
        return belongCard?.let { belongCard ->
            ...
            if (belongCard.cardType === CardTypeEnum.HERO_POWER) {
                ...
                if (powerRect.isValid()) {
                    // The old random click can land on the animated outer
                    // glow: the cursor visibly moves over 恶魔之爪, but the
                    // Unity hit-test never receives the button transition.
                    // Use the stable interior point and do not right-click
                    // cancel immediately before the hero-power click.
