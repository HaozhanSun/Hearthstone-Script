...
data class GameRect(
    ...
    /**
     * Return a stable point well inside the rectangle.  Random edge points are
     * useful for ordinary human-like clicks, but card-shaped controls often
     * have transparent borders where Hearthstone does not register a click.
     */
    fun getCenterClickPos(): Point {
        val (x, y, width, height) = getRelativeRect()
        return Point((x + width / 2.0).toInt(), (y + height / 2.0).toInt())
    }
    ...
    fun lClickCenter(isCancel: Boolean = true) {
        if (isCancel) cancel()
        showControlPos()
        GameUtil.leftButtonClick(getCenterClickPos())
    }
