...
for /f "usebackq delims=" %%J in (`powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$j = Get-ChildItem -LiteralPath '%curdir%' -Filter 'hs-script_*.jar' -File ^| Where-Object { $_.Name -notmatch '\\.before-^|\\.bak' } ^| Sort-Object LastWriteTime -Descending ^| Select-Object -First 1; if ($j) { $j.FullName }"`) do set "jarPath=%%J"
if not defined jarPath (
    echo [ERROR] No deployed hs-script JAR was found in: %curdir%
    exit /b 1
)
java -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5005 -Djna.library.path="%curdir%" -jar "%jarPath%" %1
goto end
