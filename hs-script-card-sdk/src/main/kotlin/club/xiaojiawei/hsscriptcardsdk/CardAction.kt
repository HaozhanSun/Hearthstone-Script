...
//                添加至原卡牌位置的右侧
    ...
    private fun delay() {
        if (isStop() || mouseActionInterval <= 0) return
        depth = 0
        Thread.sleep(RandomUtil.getActionInterval(mouseActionInterval).toLong())
    }

    private fun delay(time: Int) {
        if (isStop() || time <= 0) return
        depth++
        Thread.sleep(RandomUtil.getRandomAround(time, max(1, time / 3)).toLong())
    }
