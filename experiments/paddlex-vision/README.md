...
Chinese OCR on the same screenshot. The experiment remains outside the Maven
reactor. This branch also contains a production-safe PaddleX sidecar for the
high-risk rank-badge path; it is opt-in and leaves general screen/card
...
used by the opt-in production sidecar bridge on this branch. The Kotlin
detector invokes the bundled `hs-script-app/src/main/resources/paddlex/rank_bridge.py`
only when `HS_SCRIPT_RANK_OCR_ENGINE=paddlex` and
`HS_SCRIPT_PADDLEX_PYTHON` points to a separately installed PaddleX environment.
If the model or bridge fails, it records a concise fallback marker and uses
Tesseract.
...
can be exercised independently of the Maven build.
...
rank recognition. The production Kotlin detector on this branch uses the same
ROI through the bundled sidecar when PaddleX is enabled.
