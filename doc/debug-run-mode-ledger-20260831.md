...
## Follow-up harness hardening

The debug runner now resolves the application JAR from
`deployment-manifest.json` (with a timestamped-JAR fallback) instead of
hard-coding `hs-script_v4.16.3-GA.jar`. Each run writes its watchdog console
trace and a JSONL event ledger beneath `log/e2e-runs/<runId>/`; the ledger
records run start, attempt start/exit, game-ready, restart, completion, and
exhaustion. This prevents a later launch from overwriting the evidence of an
earlier launch and makes a premature JVM exit distinguishable from a clean
watchdog completion.

The most recent clean-state validation of run `11218_8096` is negative: the
fresh Power.log contained only startup/loading observations and no actual
match or terminal `PLAYSTATE` marker. It must not be counted as an E2E pass.
The source-level runner contract test is checked in, and the equivalent
PowerShell contract assertions pass. The isolated module Maven test could
not reach the test phase because the existing build requires sibling SDK and
JDBC artifacts that are not present in this worktree.
